<x-filament-panels::page>
    ...
    <div class="mt-8 bg-white dark:bg-gray-800 dark:border-gray-700 rounded-lg shadow-sm border border-gray-100 p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-100 pb-4 mb-4">
            <div>
                <h2 class="text-lg font-bold text-gray-900">My Orders</h2>
                <p class="text-xs text-gray-400 mt-1">Riwayat registrasi milik participant pada akun yang sedang aktif.</p>
            </div>
            ...
            <table class="table table-zebra">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="font-semibold text-gray-900">{{ trim(($order->participant?->firstname ?? '') . ' ' . ($order->participant?->lastname ?? '')) ?: '-' }}</div>
                        ...
                        <td>
                            <div class="font-semibold text-gray-900">{{ $order->payment?->payment_method === 'bank_transfer' ? 'Bank Transfer' : 'Online Payment' }}</div>
                        ...
                        <td>
                            <span class="badge {{ $order->payment_status === 'paid' ? 'badge-success' : 'badge-warning' }} badge-sm">
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden">
            ...
            <div class="border border-gray-100 rounded-lg p-4">
                ...
                <div class="mt-3 text-sm text-gray-700 space-y-1">
                    <p><span class="text-gray-400">Participant:</span> {{ trim(($order->participant?->firstname ?? '') . ' ' . ($order->participant?->lastname ?? '')) ?: '-' }}</p>
                    <p><span class="text-gray-400">Payment:</span> {{ $order->payment?->payment_method === 'bank_transfer' ? 'Bank Transfer' : 'Online Payment' }}</p>
                    <p><span class="text-gray-400">Total:</span> {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                </div>
            </div>
            @endforeach
        </div>
